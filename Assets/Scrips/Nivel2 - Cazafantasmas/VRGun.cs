...
public class VRGun : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Verifica que el GhostController esté asignado
        if (ghostController == null)
        {
            Debug.LogError("GhostController no asignado. Por favor, asigna el GhostController en el Inspector.");
        }
    }
    ...
    void Update()
    {
        ...
        if (leftGripValue && distanceToLeftHand <= grabDistance)
        {
            ...
            // Invoca los fantasmas al agarrar el arma
            ghostController?.SpawnGhosts();
        }
        else if (rightGripValue && distanceToRightHand <= grabDistance)
        {
            isGripped = true;
